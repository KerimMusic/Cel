...
// ------------------------------------------------------------
//  NUEVO: ¿El perfil tiene un número de WhatsApp utilizable?
//  Un número es válido si al limpiarlo quedan al menos 10 dígitos.
// ------------------------------------------------------------
function perfilTieneNumero(perfil) {
    if (!perfil) return false;

    const numeroEl = perfil.querySelector('.Whatsapp');
    const crudo = perfil.dataset.whatsapp || (numeroEl ? numeroEl.textContent : '');
    const limpio = limpiarNumero(crudo);

    return limpio.length >= 10;
}

// ------------------------------------------------------------
//  NUEVO: Refresca el estado visual de los perfiles.
//  Marca con .btn-deshabilitado los que no tienen número.
// ------------------------------------------------------------
function sincronizarPerfiles() {
    document.querySelectorAll('.perfil').forEach(function (perfil) {
        const btn = perfil.querySelector('.btn');
        const tiene = perfilTieneNumero(perfil);

        perfil.classList.toggle('perfil-sin-numero', !tiene);

        if (btn) {
            btn.classList.toggle('btn-deshabilitado', !tiene);
            btn.disabled = !tiene;
            btn.setAttribute('aria-disabled', String(!tiene));
            btn.title = tiene
                ? 'Agendar cita'
                : 'Este barbero aún no tiene número registrado';
        }
    });
}
...
function abrirModalCita(perfil) {
    ...
    // ------------------------------------------------------------
    //  VALIDACIÓN: sin número no hay cita.
    //  Se comprueba ANTES de leer el resto de datos.
    // ------------------------------------------------------------
    if (!perfilTieneNumero(perfil)) {
        mostrarToast('Este barbero aún no tiene número registrado 💈', 'error');
        return;
    }
...
function inicializarPerfiles() {
    ...
    // Estado visual inicial según tengan o no número
    sincronizarPerfiles();
    ...
    perfiles.forEach(function (perfil) {
        ...
        if (btn) {
            btn.addEventListener('click', function (e) {
                ...
                // Doble validación por si el estado cambió en caliente
                if (!perfilTieneNumero(perfil)) {
                    mostrarToast('Este barbero aún no tiene número registrado 💈', 'error');
                    return;
                }
        ...
        perfil.addEventListener('click', function (e) {
            ...
            // Sin número no se abre el modal desde la tarjeta
            if (!perfilTieneNumero(perfil)) {
                mostrarToast('Este barbero aún no tiene número registrado 💈', 'error');
                return;
            }
...
async function enviarWhatsApp() {
    ...
    // ------------------------------------------------------------
    //  VALIDACIÓN FINAL: sin número no se envía nada.
    // ------------------------------------------------------------
    if (!barberoActual.whatsapp) {
        mostrarToast('Primero elige un barbero con número registrado 💈', 'error');
        cerrarModalCita();
        const equipo = document.getElementById('equipo');
        if (equipo) equipo.scrollIntoView({ behavior: 'smooth', block: 'start' });
        return;
...
function cancelarSeguimiento(mensaje) {
    ...
    document.querySelectorAll('.perfil .btn').forEach(function (btn) {
        if (!btn.classList.contains('btn-deshabilitado')) {
...
function inicializarApp() {
    ...
    // Refresca el estado visual de los perfiles (número válido o no)
    sincronizarPerfiles();
